...
class StatRB:
    ...
    def compute_stats(self, S):
        RB = self.compute_RB(S)
        if len(RB) > 0:
            stat = list()
            stat.append(round(min(RB),2))
            stat.append(round(np.percentile(RB, 25)))
            stat.append(round(st.median(RB)))
            stat.append(round(st.mean(RB),2))
            stat.append(round(np.percentile(RB, 75),2)) 
            stat.append(max(RB))
            stat.append(round(st.stdev(RB),2))
            return stat
    ...
    def plot(self,  LIN, LIN_NM, CYC, CYC_NM, source1, source2, source3, source4, source5, source6=[],
        ...
        Database =  Database.rename_axis("RB", axis="columns")
