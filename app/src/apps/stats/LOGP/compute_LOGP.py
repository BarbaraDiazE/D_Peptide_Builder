...
class StatLOGP:
    ...
    def compute_stats(self, S):
        LOGP = self.compute_LOGP(S)
        if len(LOGP) > 0:
            stat = list()
            stat.append(round(min(LOGP),2))
            stat.append(round(np.percentile(LOGP, 25)))
            stat.append(round(st.median(LOGP)))
            stat.append(round(st.mean(LOGP),2))
            stat.append(round(np.percentile(LOGP, 75),2)) 
            stat.append(max(LOGP))
            stat.append(round(st.stdev(LOGP),2))
            return stat
    ...
    def plot(self,  LIN, LIN_NM, CYC, CYC_NM, source1, source2, source3, source4, source5, source6=[],
        ...
        Database =  Database.rename_axis("log P", axis="columns")
