...
class StatMW:
    ...
    def compute_stats(self, S):
        MW = self.compute_MW(S)
        if len(MW) > 0:
            stat = list()
            stat.append(round(min(MW),2))
            stat.append(round(np.percentile(MW, 25)))
            stat.append(round(st.median(MW)))
            stat.append(round(st.mean(MW),2))
            stat.append(round(np.percentile(MW, 75),2)) 
            stat.append(max(MW))
            stat.append(round(st.stdev(MW),2))
            return stat
    ...
    def plot(self,  LIN, LIN_NM, CYC, CYC_NM, source1, source2, source3, source4, source5, source6=[],
        ...
        Database =  Database.rename_axis("MW", axis="columns")
