...
class StatHBA:
    ...
    def compute_stats(self, S):
        HBA = self.compute_HBA(S)
        if len(HBA) > 0:
            stat = list()
            stat.append(round(min(HBA),2))
            stat.append(round(np.percentile(HBA, 25)))
            stat.append(round(st.median(HBA)))
            stat.append(round(st.mean(HBA),2))
            stat.append(round(np.percentile(HBA, 75),2)) 
            stat.append(max(HBA))
            stat.append(round(st.stdev(HBA),2))
            return stat
    ...
    def plot(self,  LIN, LIN_NM, CYC, CYC_NM, source1, source2, source3, source4, source5, source6=[],
        ...
        Database =  Database.rename_axis("HBA", axis="columns")
